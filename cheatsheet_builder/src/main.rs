...
fn main() {
    ...
    // let _app_state = AppState { service };
    _make_demo_categories_json(&service);
...
// println!("{:?}", ordered);
fn _make_demo_categories_json(s: &Service) {
    let result = s.get_categories_flat().unwrap();
    let json = serde_json::to_string(&result).unwrap();
    fs::write("./docs/json_categories.json", json).expect("couldn't write to file");
}
