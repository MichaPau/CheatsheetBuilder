...
pub fn get_categories(app_state: State<'_, AppState>) -> Result<Vec<Tag>, CheatsheetError> {
    ...
    // let cat_result = app_state.service.get_tag_list(Some(TagType::Category));
    // match cat_result {
    //     Ok(result) => {
    //         let cat_vec: Vec<Tag> = result.iter().map(|tag| tag.clone()).collect();
    //         Ok(cat_vec)
    //     },
    //     Err(e) => Err(e),
    // }
...
#[tauri::command]
pub fn set_tag_parent_id(tag_id: usize, new_parent_id: Option<usize>, app_state: State<'_, AppState>) -> Result<bool, CheatsheetError> {
